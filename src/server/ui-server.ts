...
export function createUIServer(
  ...
  httpAdapter: HttpNetworkAdapter,
  users: UserRepository,
  upstreamUrl: string,
  multiSim?: MultiDeviceSimulator,
  logger?: Logger,
): { close: () => void } {
  const app = express();
  ...
  // ─── Static dashboards ───────────────────────────────────────────────────
  app.get('/',          (_, res) => res.sendFile(path.join(process.cwd(), 'public', 'demo.html')));
  app.get('/dashboard', (_, res) => res.sendFile(path.join(process.cwd(), 'public', 'index.html')));
  app.get('/multi',     (_, res) => res.sendFile(path.join(process.cwd(), 'public', 'multi-device.html')));
  app.get('/devtools',  (_, res) => res.sendFile(path.join(process.cwd(), 'public', 'devtools.html')));
  app.get('/product',   (_, res) => res.sendFile(path.join(process.cwd(), 'public', 'product.html')));

  // ─── DevTools API ─────────────────────────────────────────────────────────
  if (logger) {
    // Log entries with optional filtering
    app.get('/api/devtools/logs', (req, res) => {
      const { level, category, search, since, limit } = req.query as Record<string, string>;
  ...
  app.get('/api/devtools/conflicts', (req, res) => {
    const { filter, limit } = req.query as Record<string, string>;
  ...
  app.post('/api/users', (req, res) => {
    const { id, name, device_id } = req.body as { id: string; name: string; device_id: string };
  ...
  app.get('/api/changes', (req, res) => {
    ...
    res.json(engine.getChanges(valid.includes(filter) ? (filter as 'all' | 'synced' | 'unsynced') : 'all', limit));
  });
  ...
  app.get('/api/conflicts/:id/preview', (req, res) => {
    ...
    if (!preview) { res.status(404).json({ error: 'conflict not found or no schema' }); return; }
    res.json(preview);
  });
  ...
  app.post('/api/conflicts/:id/auto-merge', (req, res) => {
    ...
    if (!result.ok) { res.status(400).json({ error: 'no merge schema for this entity type' }); return; }
    res.json(result);
  });
  ...
  app.post('/api/conflicts/:id/manual-merge', (req, res) => {
    ...
    if (!data || typeof data !== 'object') {
      res.status(400).json({ error: 'data object required' }); return;
    }
    const result = engine.applyManualMerge(req.params['id']!, data);
    if (!result.ok) { res.status(404).json({ error: 'conflict not found' }); return; }
    res.json(result);
  });
  ...
  app.post('/api/simulate/crash-write', (req, res) => {
    const { entityType, entityId, payload } = req.body as {
  ...
  app.post('/api/simulate/device-b-write', async (req, res) => {
    const { entityType = 'users', entityId, payload } = req.body as {
      entityType?: string;
      entityId: string;
      payload: Record<string, unknown>;
    };
    if (!entityId || !payload) {
      res.status(400).json({ error: 'entityId and payload required' }); return;
    }
  ...
  if (multiSim) {
    ...
    app.get('/api/multi/devices/:id', (req, res) => {
      ...
      if (!snap) { res.status(404).json({ error: 'device not found' }); return; }
      res.json(snap);
    });
    ...
    app.post('/api/multi/devices/:deviceId/conflicts/:conflictId/auto-merge', (req, res) => {
      ...
      if (!engine) { res.status(404).json({ error: 'device not found' }); return; }
    ...
    app.post('/api/multi/seed', async (req, res) => {
      const { users: seedUsers } = req.body as { users: { id: string; name: string }[] };
      if (!Array.isArray(seedUsers)) { res.status(400).json({ error: 'users array required' }); return; }
